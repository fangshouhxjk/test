...
path_faces_rd2 = path_faces_rd + studata.class_list[studata.class_cnt]+"/"
...
#   path_faces_StudentX:     图像文件夹的路径
#   path_csv:               要生成的 CSV 路径

def write_into_csv(path_faces_StudentX, path_csv2):
    dir_pics = os.listdir(path_faces_StudentX)
    with open(path_csv2, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        for i in range(len(dir_pics)):
            # 调用return_128d_features()得到128d特征
            print("正在读的人脸图像：", path_faces_StudentX + "/" + dir_pics[i])
            features_128d = return_128d_features(path_faces_StudentX + "/" + dir_pics[i])
            #  print(features_128d)
            # 遇到没有检测出人脸的图片跳过
            if features_128d == 0:
                i += 1
...
# 读取某人所有的人脸图像的数据，写入 Student_X.csv
faces = os.listdir(path_faces_rd2)
for Student in faces:
    print("##### " + Student + " #####")
    print(path_csv2 + Student + ".csv")
    write_into_csv(path_faces_rd2 + Student, path_csv2 + Student + ".csv")
...
# 存放所有特征均值的 CSV 的路径
path_csv_feature_all = "data/csv_feature_all/" + studata.class_list[studata.class_cnt]+"/"
if not os.path.exists(path_csv_feature_all):
    os.makedirs(path_csv_feature_all)
path_csv_feature_all += studata.class_list[studata.class_cnt]+"_features_all.csv"
# 存放人脸特征的 CSV 的路径
path_csv_rd = "data/data_csvs_from_camera/" + studata.class_list[studata.class_cnt]+"/"
